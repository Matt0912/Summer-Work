...
int convert(const char length[]) {
    int string_length = strlen(length) + 3; //Find input string length
    char length_orig[string_length];
    sprintf(length_orig, "%s\n", length); // Convert character to string so can be compared
    int length_int = strtol(length_orig, NULL, 10); //Convert to an integer
    char length_str[string_length]; // Define string to compare against original length
    sprintf(length_str, "%i\n", length_int); // Convert integer length back to a string to compare against original
    if (strcmp(length_orig, length_str) == 0 && length_int > 0) return length_int;
    else return -1;
}
